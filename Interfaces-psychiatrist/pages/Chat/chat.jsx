...
function Chat() {
  ...
  const fetchProfile = async () => {
    await axios
      .get(
        "http://127.0.0.1:8000/api/psychiatrist-chat?token=" +
          getCookie("token")
      )
      .then(({ data }) => {
        console.log(data);
  ...
  const getMessages = async (id) => {
    await axios
      .get(
        `http://127.0.0.1:8000/api/psychiatrist-getMsg?patient_id=${id}&token=${getCookie(
          "token"
        )}`
      )
      .then(({ data }) => {
  ...
  const sendMsg = async () => {
    ...
    await axios.post(
      `http://127.0.0.1:8000/api/psychiatrist-sendMsg?msg=${msg}&patient_id=${sessionStorage.getItem("id_psy")}&token=${getCookie("token")}`
    );
  };
          ...
          <div className="px-4 d-none d-md-block">
            <div className="d-flex align-items-center">
              <div className="flex-grow-1">
                <h3 className="my-3 text-primary">Chats</h3>
              </div>
            </div>
          </div>
          {myChats.length > 0 &&
          ...
          <div className="position-relative">
            <div className={css["chat-messages"] + " p-4 messages"}>
              {myMsgs.length > 0 &&
                myMsgs.map((row) => (
                  <Message
                    dir={row.sender === "patient" ? "left" : "right"}
                    time={new Date(row.date).toLocaleString("en-US", {
                      hour: "numeric",
                      minute: "numeric",
                      hour12: true,
                    })}
